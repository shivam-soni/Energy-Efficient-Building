# -*- coding: utf-8 -*-
"""
Created on Fri Nov  9 01:11:14 2018
...
y1=y_train.iloc[:,0]
y1=pd.DataFrame(y1,columns=['heating_load'])

y2=y_train.iloc[:,1]
y2=pd.DataFrame(y2,columns=['cooling_load'])
#Feature Selection
model = ExtraTreesRegressor()


rfe = RFE(model, 5)
fit = rfe.fit(x_train,y1)
print("For Y1----------------------------------\n")
print("Number of Features: ", fit.n_features_)
print("Selected Features: ", fit.support_)
print("Feature Ranking: ", fit.ranking_) 
indices_selected_y1= fit.get_support(indices=True)
columns_selected_y1= [X.columns[i] for i in indices_selected_y1]
print("For y1 best features",columns_selected_y1)
...
print("Selected Features: ", fit.support_)
print("Feature Ranking: ", fit.ranking_) 
indices_selected_y2= fit.get_support(indices=True)
columns_selected_y2= [X.columns[i] for i in indices_selected_y1]
print("For y1 best features",columns_selected_y2)

feature_x_train= x_train[columns_selected_y2]
feature_x_test=x_test[columns_selected_y2]
...
m=reg.fit(feature_x_train,y_train)
pred1=m.predict(feature_x_train)
pred=m.predict(feature_x_test)
...
201) 0.923 (+/-0.433) for {'activation': 'tanh', 'alpha': 1e-05, 'hidden_layer_sizes': 25, 'learning_rate': 'constant', 'learning_rate_init': 0.001, 'solver': 'adam'}
...
210) 0.876 (+/-0.534) for {'activation': 'tanh', 'alpha': 1e-06, 'hidden_layer_sizes': 25, 'learning_rate': 'constant', 'learning_rate_init': 0.001, 'solver': 'adam'}
