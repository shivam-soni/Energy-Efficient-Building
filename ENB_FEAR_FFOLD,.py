# -*- coding: utf-8 -*-
"""
Created on Fri Nov  9 02:25:58 2018
...
X1=X.drop(num_cols,1)
X=pd.concat([X1.reset_index(drop=True),X_transformed_df.reset_index(drop=True) ],axis=1)
...
indices_selected_y= fit.get_support(indices=True)
columns_selected_y= [X.columns[i] for i in indices_selected_y]
print("For y1 best features",columns_selected_y)
...
columns_selected_y2= [X.columns[i] for i in indices_selected_y2]
print("For y1 best features",columns_selected_y2)

feature_X= X[columns_selected_y2]
